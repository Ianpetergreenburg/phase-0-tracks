...
it "checks if whatever is passed is a string of only letters" do
  expect(wordgame.isword?("cc")).to eq true
  expect(wordgame.isword?("Test")).to eq true
  expect(wordgame.isword?("Test4")).to eq false
  expect(wordgame.isword?("c c")).to eq false
  expect(wordgame.isword?("")).to eq false
end

  it "checks if whatever is passed is a single letter" do
   expect(wordgame.isletter?("c")).to eq true
   expect(wordgame.isletter?("bc")).to eq false
   expect(wordgame.isletter?(" ")).to eq false
   expect(wordgame.isletter?("4")).to eq false
   expect(wordgame.isletter?("C")).to eq true
end
